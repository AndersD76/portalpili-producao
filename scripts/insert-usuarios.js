...
async function insertUsuarios() {
  ...
  try {
    ...
    // Limpar tabela de usuários primeiro (cuidado!)
    await client.query('DELETE FROM usuarios WHERE id < 200');
    ...
    for (const usuario of usuarios) {
      try {
        ...
        // Gerar id_funcionario (usando o ID como string com formato USR###)
        const idFuncionario = `USR${String(usuario.id).padStart(3, '0')}`;
        ...
        `, [
          usuario.id,
          usuario.nome,
          `${usuario.nome.toLowerCase().replace(/\s+/g, '.')}@pili.com`,
          idFuncionario,
          senhaHash,
          'Funcionário',
          'Produção',
    ...
    console.log(`\n🎉 Concluído!`);
    console.log(`   ✓ ${sucessos} usuários inseridos com sucesso`);
    if (erros > 0) {
