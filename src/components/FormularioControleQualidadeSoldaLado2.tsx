...
export default function FormularioControleQualidadeSoldaLado2({
  ...
  // Carregar opções das perguntas do banco de dados (Setor E - Solda Lado 2)
  useEffect(() => {
    let isMounted = true;

    const carregarPerguntasDB = async () => {
      try {
        const response = await fetch('/api/qualidade/cq-config/perguntas-setor/E');
        if (!isMounted) return;
        ...
        console.error('[CQ-SoldaLado2] ERRO ao carregar perguntas:', err);
        if (isMounted) {
          setLoadingOpcoes(false);
        }
      }
  ...
  const renderCheckpoint = (
    ...
    // Extrair código da pergunta (ex: cq3e -> CQ3-E)
