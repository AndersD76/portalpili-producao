...
export default function FormularioPainelEletrico({ opd, cliente, atividadeId, onSubmit, onCancel }: FormularioPainelEletricoProps) {
  ...
  useEffect(() => {
    const carregarDadosExistentes = async () => {
      ...
      try {
        const response = await fetch(`/api/formularios-painel-eletrico/${opd}?atividade_id=${atividadeId}`);
  ...
  useEffect(() => {
    const carregarPerguntasDB = async () => {
      try {
        const response = await fetch('/api/qualidade/cq-config/perguntas-setor/N');
        ...
        console.error('[FormularioPainelEletrico] Erro ao carregar perguntas:', err);
      }
    };
  ...
  const renderCQField = (label: string, fieldName: string, criterio: string, hasNaoAplicavel = false) => {
    // Extrair codigo da pergunta do fieldName (ex: cq1n -> CQ1-N)
