...
export default function FormularioColetorMontagemInicial({ opd, cliente, atividadeId, onSubmit, onCancel }: FormularioColetorMontagemInicialProps) {
  ...
  useEffect(() => {
    const carregarDadosExistentes = async () => {
      ...
      try {
        const response = await fetch(`/api/formularios-coletor-montagem-inicial/${opd}?atividade_id=${atividadeId}`);
  ...
  // Carregar opções das perguntas do banco de dados (Setor Ac)
  useEffect(() => {
    let isMounted = true;
    const carregarPerguntasDB = async () => {
      try {
        const response = await fetch('/api/qualidade/cq-config/perguntas-setor/Ac');
        if (!isMounted) return;
        ...
        console.error('[CQ-SectorAc] ERRO ao carregar perguntas:', err);
