...
export interface FormularioBaseProps {
  ...
  // Suporte a salvamento de rascunho
  mostrarBotaoRascunho?: boolean;
  savingDraft?: boolean;
  onSalvarRascunho?: () => void;
  isRascunhoExistente?: boolean;
...
export default function FormularioBase({
  ...
  mostrarBotaoRascunho = false,
  savingDraft = false,
  onSalvarRascunho,
  isRascunhoExistente = false,
  loadingDados = false,
}: FormularioBaseProps) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6 max-h-[80vh] overflow-y-auto px-1">
      ...
      {/* Botões */}
      <div className="flex flex-col sm:flex-row justify-end gap-2 sm:space-x-3 pt-4 border-t sticky bottom-0 bg-white py-4">
        <button
          type="button"
          onClick={onCancel}
        ...
        {mostrarBotaoRascunho && onSalvarRascunho && (
          <button
            type="button"
        ...
        <button
          type="submit"
          className={`px-4 py-2 ${mostrarBotaoRascunho ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'} text-white rounded-lg transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2 text-sm`}
          disabled={loading || savingDraft}
        >
          {loading ? (
            <>
              <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
              <span>{mostrarBotaoRascunho ? 'Finalizando...' : 'Salvando...'}</span>
            </>
          ) : (
            <>
              {mostrarBotaoRascunho && (
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                </svg>
              )}
              <span>{mostrarBotaoRascunho ? 'Finalizar' : botaoSalvarTexto}</span>
            </>
          )}
        </button>
      </div>
