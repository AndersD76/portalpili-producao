...
interface PageHeaderProps {
  ...
  backHref: string;
  navLinks?: NavLink[];
  /** Botões extras no lado direito, antes do logout */
  rightExtra?: React.ReactNode;
}

export function PageHeader({ title, backHref, navLinks = [], rightExtra }: PageHeaderProps) {
          ...
          <div className="flex items-center gap-1">
            {navLinks.map(link => (
              <Link
                key={link.href}
                href={link.href}
                className="hidden sm:inline-flex items-center gap-1 px-2 py-1 text-xs sm:text-sm text-gray-500 hover:text-red-600 hover:bg-gray-50 rounded-lg transition"
              >
...
// ─── Nav presets por módulo ───────────────────────────────────────────────────

export const COMERCIAL_NAV: NavLink[] = [
  {
  ...
  {
    href: '/comercial/configurador',
    label: 'Orçamento',
    icon: 'M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z',
  },
  {
    href: '/comercial/materiais',
    label: 'Materiais',
    icon: 'M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z',
  },
];

export const QUALIDADE_NAV: NavLink[] = [
  {
    href: '/qualidade',
    label: 'Qualidade',
