...
export default function FormularioSobPlataforma({ opd, cliente, atividadeId, onSubmit, onCancel }: FormularioSobPlataformaProps) {
  ...
  useEffect(() => {
    const carregarDadosExistentes = async () => {
      ...
      try {
        const response = await fetch(`/api/formularios-sob-plataforma/${opd}?atividade_id=${atividadeId}`);
  ...
  // Carregar opções das perguntas do banco de dados (Setor P)
  useEffect(() => {
    let isMounted = true;
    const carregarPerguntasDB = async () => {
      try {
        const response = await fetch('/api/qualidade/cq-config/perguntas-setor/P');
        if (!isMounted) return;
        ...
        console.error('[CQ-SetorP] ERRO ao carregar perguntas:', err);
