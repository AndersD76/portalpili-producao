...
const ATIVIDADE_TO_FORM: { [key: string]: string } = {
  ...
  // Produção - nomes com prefixo (formato atual)
  'A - CORTE': 'CORTE',
  'B - MONTAGEM SUPERIOR E ESQUADRO': 'MONTAGEM',
  'C - CENTRAL HIDRÁULICA (SETOR HIDRÁULICA)': 'CENTRAL_SUBCONJUNTOS',
  'D - SOLDA LADO 01': 'CONTROLE_QUALIDADE_SOLDA',
  'E - SOLDA LADO 02': 'CONTROLE_QUALIDADE_SOLDA_LADO2',
  'F - MONTAGEM E SOLDA INFERIOR': 'MONTAGEM_SOLDA_INFERIOR',
  'G - MONTAGEM ELÉTRICA/HIDRÁULICO': 'MONTAGEM_ELETRICA_HIDRAULICA',
  'H - MONTAGEM DAS CALHAS': 'MONTAGEM_CALHAS',
  'I - TRAVADOR DE RODAS LATERAL MÓVEL': 'TRAVADOR_RODAS',
  'J - CAIXA DO TRAVA CHASSI': 'CAIXA_TRAVA_CHASSI',
  'K - TRAVA CHASSI': 'TRAVA_CHASSI',
  'L - CAVALETE DO TRAVA CHASSI': 'CAVALETE_TRAVA_CHASSI',
  'M - CENTRAL HIDRÁULICA (SETOR SUBCONJUNTOS)': 'CENTRAL_SUBCONJUNTOS',
  'N - PAINEL ELÉTRICO': 'PAINEL_ELETRICO',
  'O - PEDESTAIS': 'PEDESTAIS',
  'P - SOB PLATAFORMA': 'SOB_PLATAFORMA',
  'Q - SOLDA INFERIOR': 'SOLDA_INFERIOR',
  'R - BRAÇOS': 'BRACOS',
  'S - RAMPAS': 'RAMPAS',
  'T - PINTURA E PREPARAÇÃO DA PLATAFORMA': 'PINTURA',
  'U - MONTAGEM HIDRÁULICA ELÉTRICA SOB PLATAFORMA': 'MONTAGEM_HIDRAULICA_SOB_PLATAFORMA',
  // Produção - nomes sem prefixo (fallback)
  'CORTE': 'CORTE',
      ...
      >
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center space-x-2 sm:space-x-4 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1 sm:gap-2 flex-wrap">
                ...
                {/* Badge de Pendência no Formulário */}
                {(atividade as any).tem_pendencia_formulario && (
                  <span className="px-2 py-0.5 bg-orange-100 text-orange-700 text-xs rounded-full font-bold flex items-center gap-1">
                    <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
          ...
          <div className="flex items-center space-x-2 sm:space-x-3 flex-shrink-0">
            ...
            {/* Botão de preencher formulário - aparece quando em andamento ou pausada */}
            {FormComponent && (atividade.status === 'EM ANDAMENTO' || atividade.status === 'PAUSADA') && (
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  setShowFormModal(true);
