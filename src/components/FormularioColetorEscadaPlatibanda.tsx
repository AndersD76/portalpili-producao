...
export default function FormularioColetorEscadaPlatibanda({ opd, cliente, atividadeId, onSubmit, onCancel }: FormularioColetorEscadaPlatibandaProps) {
  ...
  useEffect(() => {
    const carregarDadosExistentes = async () => {
      ...
      try {
        const response = await fetch(`/api/formularios-coletor-escada-platibanda/${opd}?atividade_id=${atividadeId}`);
  ...
  // Carregar opções das perguntas do banco de dados (Setor Gc)
  useEffect(() => {
    let isMounted = true;
    const carregarPerguntasDB = async () => {
      try {
        const response = await fetch('/api/qualidade/cq-config/perguntas-setor/Gc');
        if (!isMounted) return;
        ...
        console.error('[CQ-SectorGc] ERRO ao carregar perguntas:', err);
