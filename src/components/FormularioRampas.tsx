...
export default function FormularioRampas({ opd, cliente, atividadeId, onSubmit, onCancel }: FormularioRampasProps) {
  ...
  useEffect(() => {
    const carregarDadosExistentes = async () => {
      ...
      try {
        const response = await fetch(`/api/formularios-rampas/${opd}?atividade_id=${atividadeId}`);
  ...
  useEffect(() => {
    const carregarPerguntasDB = async () => {
      try {
        const response = await fetch('/api/qualidade/cq-config/perguntas-setor/S');
        ...
        console.error('[FormularioRampas] Erro ao carregar perguntas:', err);
      }
    };
  ...
  const renderCQField = (label: string, fieldName: string, criterio: string, hasNaoAplicavel = false) => {
    // Extrair codigo da pergunta do fieldName (ex: cq1s -> CQ1-S)
