...
export default function FormularioMontagem({ opd, cliente, atividadeId, onSubmit, onCancel }: FormularioMontagemProps) {
  ...
  useEffect(() => {
    const carregarDadosExistentes = async () => {
      ...
      try {
        const response = await fetch(`/api/formularios-montagem/${opd}?atividade_id=${atividadeId}`);
        ...
        if (result.success && result.data?.dados_formulario) {
          ...
          setFormData(prev => ({ ...prev, ...dadosFormulario }));
        }
      } catch (err) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, fieldName: string) => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        formData.append('tipo', 'controle_qualidade_montagem');
  ...
  const handleSalvarRascunho = async () => {
    ...
    try {
      const response = await fetch(`/api/formularios-montagem/${opd}`, {
        method: 'POST',
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-h-[80vh] overflow-y-auto px-1">
      ...
      <div className="bg-gray-50 p-4 rounded-lg border-2 border-gray-300">
        <h3 className="font-bold text-lg mb-2">Controle de Qualidade - Montagem Superior e Esquadro - OPD: {opd}</h3>
