...
export default function FormularioCaixaTravaChassi({ opd, cliente, atividadeId, onSubmit, onCancel }: FormularioCaixaTravaChassiProps) {
  ...
  useEffect(() => {
    const carregarDadosExistentes = async () => {
      ...
      try {
        const response = await fetch(`/api/formularios-caixa-trava-chassi/${opd}?atividade_id=${atividadeId}`);
  ...
  useEffect(() => {
    const carregarPerguntasDB = async () => {
      try {
        const response = await fetch('/api/qualidade/cq-config/perguntas-setor/J');
        ...
        console.error('[FormularioCaixaTravaChassi] Erro ao carregar perguntas:', err);
      }
    };
  ...
  const renderCQField = (label: string, fieldName: string, criterio: string, hasNaoAplicavel = false) => {
    // Extrair codigo da pergunta do fieldName (ex: cq1j -> CQ1-J)
