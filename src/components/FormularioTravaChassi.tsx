...
interface FormularioTravaChassiProps {
  opd: string;
  cliente: string;
  atividadeId?: number;
...
export default function FormularioTravaChassi({ opd, cliente, atividadeId, onSubmit, onCancel }: FormularioTravaChassiProps) {
  ...
  const [formData, setFormData] = useState({
    cq1k_status: '', // VERIFICAR MEDIDA INTERNA DA CAIXA TRAVA CHASSI
    cq2k_status: '', // TESTAR PINO DO CILINDRO
    cq3k_status: '', // PARAFUSOS ESTÃO FIXADOS
    cq4k_status: '', // SOLDA CAVALETE TRAVA CHASSI
  });
  ...
  useEffect(() => {
    const carregarDadosExistentes = async () => {
      ...
      try {
        const response = await fetch(`/api/formularios-trava-chassi/${opd}?atividade_id=${atividadeId}`);
  ...
  useEffect(() => {
    const carregarPerguntasDB = async () => {
      try {
        const response = await fetch('/api/qualidade/cq-config/perguntas-setor/K');
        ...
        console.error('[FormularioTravaChassi] Erro ao carregar perguntas:', err);
      }
    };
  ...
  const handleSalvarRascunho = async () => {
    ...
    try {
      const response = await fetch(`/api/formularios-trava-chassi/${opd}`, {
        method: 'POST',
        headers: {
  ...
  const renderCQField = (label: string, fieldName: string, criterio: string, hasNaoAplicavel = false) => {
    // Extrair codigo da pergunta do fieldName (ex: cq1k -> CQ1-K)
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-h-[80vh] overflow-y-auto px-1">
      ...
      <div className="bg-gray-50 p-4 rounded-lg border-2 border-gray-300">
        <h3 className="font-bold text-lg mb-2">CQ-K: TRAVA CHASSI - OPD: {opd}</h3>
        <p className="text-gray-700">CLIENTE: {cliente}</p>
      </div>

      <div className="border-2 border-red-300 rounded-lg p-4 bg-red-50">
        <h4 className="font-bold text-xl mb-4 text-red-900">K - TRAVA CHASSI</h4>

        {renderCQField('CQ1-K: VERIFICAR MEDIDA INTERNA DA CAIXA TRAVA CHASSI', 'cq1k', '+/- 5 mm')}
        {renderCQField('CQ2-K: TESTAR PINO DO CILINDRO', 'cq2k', 'Funcional')}
        {renderCQField('CQ3-K: PARAFUSOS ESTÃO FIXADOS', 'cq3k', 'Conforme torque especificado')}
        {renderCQField('CQ4-K: SOLDA CAVALETE TRAVA CHASSI', 'cq4k', 'Sem trinca; Sem porosidade')}
      </div>
