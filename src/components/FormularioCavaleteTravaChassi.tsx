...
export default function FormularioCavaleteTravaChassi({ opd, cliente, atividadeId, onSubmit, onCancel }: FormularioCavaleteTravaChassiProps) {
  ...
  useEffect(() => {
    const carregarDadosExistentes = async () => {
      ...
      try {
        const response = await fetch(`/api/formularios-cavalete-trava-chassi/${opd}?atividade_id=${atividadeId}`);
  ...
  useEffect(() => {
    const carregarPerguntasDB = async () => {
      try {
        const response = await fetch('/api/qualidade/cq-config/perguntas-setor/L');
        ...
        console.error('[FormularioCavaleteTravaChassi] Erro ao carregar perguntas:', err);
      }
    };
  ...
  const renderCQField = (label: string, fieldName: string, criterio: string, hasNaoAplicavel = false) => {
    // Extrair codigo da pergunta do fieldName (ex: cq1l -> CQ1-L)
