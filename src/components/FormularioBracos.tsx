...
export default function FormularioBracos({ opd, cliente, atividadeId, onSubmit, onCancel }: FormularioBracosProps) {
  ...
  useEffect(() => {
    const carregarPerguntasDB = async () => {
      try {
        const response = await fetch('/api/qualidade/cq-config/perguntas-setor/R');
        ...
        console.error('[FormularioBracos] Erro ao carregar perguntas:', err);
      }
    };
  ...
  const renderCQField = (label: string, fieldName: string, criterio: string, hasNaoAplicavel = false) => {
    // Extrair codigo da pergunta do fieldName (ex: cq1r -> CQ1-R)
