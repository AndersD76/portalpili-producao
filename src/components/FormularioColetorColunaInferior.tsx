...
interface FormularioColetorColunaInferiorProps {
  opd: string;
  cliente: string;
  atividadeId?: number;
...
export default function FormularioColetorColunaInferior({ opd, cliente, atividadeId, onSubmit, onCancel }: FormularioColetorColunaInferiorProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    cq2ec_status: '', // 11 PARAFUSOS JÁ ROSCADOS M22X2" COM PORCA 8.8
    cq3ec_status: '', // FURAÇÃO DO FLANGE CHUMBADOR NO DIAMETRO 23mm
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/formularios-coletor-coluna-inferior/${opd}`, {
        method: 'POST',
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-h-[80vh] overflow-y-auto px-1">
      ...
      <div className="bg-green-50 p-4 rounded-lg border-2 border-green-300">
        <h3 className="font-bold text-lg mb-2">CQ-Ec: COLUNA INFERIOR (COLETOR) - OPD: {opd}</h3>
      ...
      <div className="border-2 border-green-300 rounded-lg p-4 bg-green-50">
        <h4 className="font-bold text-xl mb-4 text-green-900">Ec - COLUNA INFERIOR</h4>

        {renderCQField('CQ1-Ec: FIXADORES DAS MANGUEIRAS HIDRÁULICAS (2 em um lado e 1 outro)', 'cq1ec', 'Conforme especificação')}
        {renderCQField('CQ2-Ec: 11 PARAFUSOS JÁ ROSCADOS M22X2" COM PORCA 8.8', 'cq2ec', '11 parafusos presentes')}
        {renderCQField('CQ3-Ec: FURAÇÃO DO FLANGE CHUMBADOR NO DIÂMETRO 23mm', 'cq3ec', '23mm')}
      </div>
      ...
      <div className="flex justify-end space-x-3 pt-4 border-t sticky bottom-0 bg-white">
        ...
        <button type="submit" className="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 flex items-center space-x-2" disabled={loading}>
          {loading ? (<><div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div><span>Salvando...</span></>) : (<span>Salvar CQ-Ec</span>)}
        </button>
      </div>
    </form>
  );
