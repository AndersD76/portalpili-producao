...
export default function ComercialVendedoresPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setFormData({ nome: '', email: '', telefone: '', whatsapp: '', cargo: 'VENDEDOR', comissao_padrao: '0.048', tipo: 'REPRESENTANTE' });
        loadVendedores();
      } else {
  ...
  const fmtCurrency = (v: unknown) =>
    new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL', minimumFractionDigits: 0 }).format(toNum(v));
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600 font-medium">Carregando vendedores...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-14 sm:h-16">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg sm:text-xl font-bold text-gray-900">Vendedores</h1>
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Valor Ganho</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Status</th>
                {isAdmin && <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Acoes</th>}
              </tr>
            </thead>
              ...
              {filteredVendedores.map((vendedor) => (
                <tr key={vendedor.id} className={`hover:bg-gray-50 ${!vendedor.ativo ? 'opacity-50' : ''}`}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-green-600 font-medium">
                    {fmtCurrency(vendedor.valor_total_ganho)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    {isAdmin ? (
                      <button
