...
// POST - Salvar formulário de controle de qualidade central
export async function POST(
  ...
  try {
    ...
    const existingResult = await client.query(`
      ...
      WHERE atividade_id = $1 AND tipo_formulario = 'CONTROLE_QUALIDADE_CENTRAL'
      LIMIT 1
    `, [atividade_id]);
    ...
    return NextResponse.json({
      ...
      message: is_rascunho
        ...
        : 'Formulário de controle de qualidade central salvo com sucesso'
    }, { status: existingResult.rows.length > 0 ? 200 : 201 });
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('Erro ao salvar formulário de controle de qualidade central:', error);
    return NextResponse.json(
      { success: false, error: 'Erro ao salvar formulário de controle de qualidade central' },
      { status: 500 }
    );
  } finally {
    client.release();
...
// GET - Buscar formulário de controle de qualidade central por atividade_id ou numero_opd
export async function GET(
  ...
  try {
    ...
    let query = `
      ...
      WHERE tipo_formulario = 'CONTROLE_QUALIDADE_CENTRAL'
    `;
    ...
    console.error('Erro ao buscar formulário de controle de qualidade central:', error);
    return NextResponse.json(
      { success: false, error: 'Erro ao buscar formulário de controle de qualidade central' },
      { status: 500 }
    );
  }
}
