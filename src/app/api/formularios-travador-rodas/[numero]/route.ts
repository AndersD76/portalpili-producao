...
// POST - Salvar formulário de controle de qualidade - Travador de Rodas
export async function POST(
  ...
  try {
    ...
    const existingResult = await client.query(`
      ...
      WHERE atividade_id = $1 AND tipo_formulario = 'CONTROLE_QUALIDADE_TRAVADOR_RODAS'
      LIMIT 1
    `, [atividade_id]);
    ...
    if (existingResult.rows.length > 0) {
      ...
      `, [
        atividade_id || null,
        numero,
        'CONTROLE_QUALIDADE_TRAVADOR_RODAS',
        JSON.stringify({ ...dados_formulario, _is_rascunho: is_rascunho }),
        null,
        preenchido_por || 'Sistema',
    ...
    return NextResponse.json({
      ...
      message: is_rascunho
        ...
        : 'Formulário de controle de qualidade - Travador de Rodas salvo com sucesso'
    }, { status: existingResult.rows.length > 0 ? 200 : 201 });
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('Erro ao salvar formulário de controle de qualidade - Travador de Rodas:', error);
    return NextResponse.json(
      { success: false, error: 'Erro ao salvar formulário de controle de qualidade - Travador de Rodas' },
      { status: 500 }
    );
  } finally {
    client.release();
...
// GET - Buscar formulário de controle de qualidade - Travador de Rodas por atividade_id ou numero_opd
export async function GET(
  ...
  try {
    ...
    let query = `
      ...
      WHERE tipo_formulario = 'CONTROLE_QUALIDADE_TRAVADOR_RODAS'
    `;
    ...
    console.error('Erro ao buscar formulário de controle de qualidade - Travador de Rodas:', error);
    return NextResponse.json(
      { success: false, error: 'Erro ao buscar formulário de controle de qualidade - Travador de Rodas' },
      { status: 500 }
    );
  }
}
