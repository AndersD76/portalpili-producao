...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json({
      ...
      message: 'Formulário de entrega salvo com sucesso'
    }, { status: 201 });
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('Erro ao salvar formulário de entrega:', error);
    return NextResponse.json(
      { success: false, error: 'Erro ao salvar formulário de entrega' },
      { status: 500 }
    );
  } finally {
    client.release();
...
// GET - Buscar formulário de entrega
export async function GET(
  ...
  try {
    ...
    console.error('Erro ao buscar formulário de entrega:', error);
    return NextResponse.json(
      { success: false, error: 'Erro ao buscar formulário de entrega' },
      { status: 500 }
    );
  }
}
