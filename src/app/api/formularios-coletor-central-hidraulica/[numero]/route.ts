...
export async function POST(request: Request, { params }: { params: Promise<{ numero: string }> }) {
  ...
  try {
    ...
    const result = await client.query(`INSERT INTO formularios_preenchidos (atividade_id, numero_opd, tipo_formulario, dados_formulario, anexos, preenchido_por, data_preenchimento, created, updated) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *`, [atividade_id || null, numero, 'CONTROLE_QUALIDADE_COLETOR_CENTRAL_HIDRAULICA', JSON.stringify(dados_formulario), null, preenchido_por || 'Sistema', new Date().toISOString(), new Date().toISOString(), new Date().toISOString()]);
...
export async function GET(request: Request, { params }: { params: Promise<{ numero: string }> }) {
  try {
    ...
    let query = `SELECT * FROM formularios_preenchidos WHERE tipo_formulario = 'CONTROLE_QUALIDADE_COLETOR_CENTRAL_HIDRAULICA'`;
