...
// POST - Salvar formulário de preparação (sem alterar status da atividade)
export async function POST(
  ...
  try {
    ...
    // Inserir formulário de preparação
    ...
    return NextResponse.json({
      ...
      message: 'Formulário de preparação salvo com sucesso'
    }, { status: 201 });
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('Erro ao salvar formulário de preparação:', error);
    return NextResponse.json(
      { success: false, error: 'Erro ao salvar formulário de preparação' },
      { status: 500 }
    );
  } finally {
    client.release();
...
// GET - Buscar formulário de preparação por atividade_id ou numero_opd
export async function GET(
  ...
  try {
    ...
    let query = `
      ...
      WHERE tipo_formulario = 'PREPARACAO'
    `;
    ...
    console.error('Erro ao buscar formulário de preparação:', error);
    return NextResponse.json(
      { success: false, error: 'Erro ao buscar formulário de preparação' },
      { status: 500 }
    );
  }
}
