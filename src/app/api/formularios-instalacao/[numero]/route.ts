...
// POST - Salvar formulário de instalação
export async function POST(
  ...
  try {
    ...
    return NextResponse.json({
      ...
      message: 'Formulário de instalação salvo com sucesso'
    }, { status: 201 });
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('Erro ao salvar formulário de instalação:', error);
    return NextResponse.json(
      { success: false, error: 'Erro ao salvar formulário de instalação' },
      { status: 500 }
    );
  } finally {
    client.release();
...
// GET - Buscar formulário de instalação
export async function GET(
  ...
  try {
    ...
    let query = `
      ...
      WHERE tipo_formulario = 'INSTALACAO'
    `;
    ...
    console.error('Erro ao buscar formulário de instalação:', error);
    return NextResponse.json(
      { success: false, error: 'Erro ao buscar formulário de instalação' },
      { status: 500 }
    );
  }
}
