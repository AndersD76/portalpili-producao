...
export async function POST(
  ...
  try {
    ...
    const existingResult = await client.query(`
      ...
      WHERE atividade_id = $1 AND tipo_formulario = 'CONTROLE_QUALIDADE_CAVALETE_TRAVA_CHASSI'
      LIMIT 1
    `, [atividade_id]);
    ...
    if (existingResult.rows.length > 0) {
      ...
      `, [
        atividade_id || null,
        numero,
        'CONTROLE_QUALIDADE_CAVALETE_TRAVA_CHASSI',
        JSON.stringify({ ...dados_formulario, _is_rascunho: is_rascunho }),
        null,
        preenchido_por || 'Sistema',
    ...
    return NextResponse.json({
      ...
      message: is_rascunho
        ...
        : 'Formulário CQ-L salvo com sucesso'
