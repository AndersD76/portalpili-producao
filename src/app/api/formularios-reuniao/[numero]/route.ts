...
// POST - Salvar formulário de reunião (sem alterar status da atividade)
export async function POST(
  ...
  try {
    ...
    return NextResponse.json({
      ...
      message: 'Formulário de reunião salvo com sucesso'
    }, { status: 201 });
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('Erro ao salvar formulário de reunião:', error);
    return NextResponse.json(
      { success: false, error: 'Erro ao salvar formulário de reunião' },
      { status: 500 }
    );
  } finally {
    client.release();
...
// GET - Buscar formulário de reunião por atividade_id ou numero_opd
export async function GET(
  ...
  try {
    ...
    console.error('Erro ao buscar formulário de reunião:', error);
    return NextResponse.json(
      { success: false, error: 'Erro ao buscar formulário de reunião' },
      { status: 500 }
    );
  }
}
