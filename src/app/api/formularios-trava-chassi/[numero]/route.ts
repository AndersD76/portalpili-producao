...
export async function POST(
  ...
  try {
    ...
    `, [atividade_id || null, numero, 'CONTROLE_QUALIDADE_TRAVA_CHASSI', JSON.stringify(dados_formulario), null, preenchido_por || 'Sistema', new Date().toISOString(), new Date().toISOString(), new Date().toISOString()]);

    if (atividade_id) { await verificarNaoConformidade(atividade_id, dados_formulario); }
    await client.query('COMMIT');
    return NextResponse.json({ success: true, data: formularioResult.rows[0], message: 'Formulário CQ-K salvo com sucesso' }, { status: 201 });
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('Erro ao salvar formulário CQ-K:', error);
...
export async function GET(
  ...
  try {
    ...
    let query = `SELECT * FROM formularios_preenchidos WHERE tipo_formulario = 'CONTROLE_QUALIDADE_TRAVA_CHASSI'`;
    ...
    console.error('Erro ao buscar formulário CQ-K:', error);
