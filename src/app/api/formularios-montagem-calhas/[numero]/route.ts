...
// POST - Salvar formulário de controle de qualidade - Montagem das Calhas
export async function POST(
  ...
  try {
    ...
    const existingResult = await client.query(`
      ...
      WHERE atividade_id = $1 AND tipo_formulario = 'CONTROLE_QUALIDADE_MONTAGEM_CALHAS'
      LIMIT 1
    `, [atividade_id]);
    ...
    if (existingResult.rows.length > 0) {
      ...
      `, [
        atividade_id || null,
        numero,
        'CONTROLE_QUALIDADE_MONTAGEM_CALHAS',
        JSON.stringify({ ...dados_formulario, _is_rascunho: is_rascunho }),
        null,
        preenchido_por || 'Sistema',
    ...
    return NextResponse.json({
      ...
      message: is_rascunho
        ...
        : 'Formulário de controle de qualidade - Montagem das Calhas salvo com sucesso'
    }, { status: existingResult.rows.length > 0 ? 200 : 201 });
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('Erro ao salvar formulário de controle de qualidade - Montagem das Calhas:', error);
    return NextResponse.json(
      { success: false, error: 'Erro ao salvar formulário de controle de qualidade - Montagem das Calhas' },
      { status: 500 }
    );
  } finally {
    client.release();
...
// GET - Buscar formulário de controle de qualidade - Montagem das Calhas por atividade_id ou numero_opd
export async function GET(
  ...
  try {
    ...
    let query = `
      ...
      WHERE tipo_formulario = 'CONTROLE_QUALIDADE_MONTAGEM_CALHAS'
    `;
    ...
    console.error('Erro ao buscar formulário de controle de qualidade - Montagem das Calhas:', error);
    return NextResponse.json(
      { success: false, error: 'Erro ao buscar formulário de controle de qualidade - Montagem das Calhas' },
      { status: 500 }
    );
  }
}
