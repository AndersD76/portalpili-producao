...
// POST - Salvar formulário de desembarque e pré-instalação
export async function POST(
  ...
  try {
    ...
    return NextResponse.json({
      ...
      message: 'Formulário de desembarque e pré-instalação salvo com sucesso'
    }, { status: 201 });
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('Erro ao salvar formulário de desembarque:', error);
    return NextResponse.json(
      { success: false, error: 'Erro ao salvar formulário de desembarque' },
      { status: 500 }
    );
  } finally {
    client.release();
...
// GET - Buscar formulário de desembarque
export async function GET(
  ...
  try {
    ...
    let query = `
      ...
      WHERE tipo_formulario = 'DESEMBARQUE_PRE_INSTALACAO'
    `;
    ...
    console.error('Erro ao buscar formulário de desembarque:', error);
    return NextResponse.json(
      { success: false, error: 'Erro ao buscar formulário de desembarque' },
      { status: 500 }
    );
  }
}
